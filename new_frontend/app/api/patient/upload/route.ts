...
export async function POST(req: NextRequest) {
  try {
    ...
    const audits = db.collection<AuditLog>("auditLogs")
    ...
    const newRecord: Omit<MedicalRecord, "_id"> = {
      ...
      fileUrl: url,
      fileCID: cid,
      fileHash: cid, // CID acts as content hash (prototype)
      recordType,
      uploadDate: new Date(),
      metadata: { description },
    }

    const result = await records.insertOne(newRecord as any)
