...
// REPLACE: new_frontend/app/api/doctor/records/[patientId]/route.ts
...
export async function GET(
  ...
  try {
    const user = requireVerified(req)
    console.log(`[Doctor Records API] Doctor ${user.userId} fetching patient records`)
    ...
    const doctorChain = doctorDoc?.blockchainAddress?.toLowerCase() ?? ""
    console.log(`[Doctor Records API] Doctor blockchain: ${doctorChain}`)
    ...
    if (!permission) {
      console.warn(`[Doctor Records API] No view permission for doctor ${user.userId} on patient ${patientId}`)
    ...
    console.log(`[Doctor Records API] Permission verified, access level: ${permission.accessLevel}`)
    ...
    console.log(`[Doctor Records API] Found ${records.length} records`)
    ...
    const mapped = records.map((rec) => {
      ...
      if (!myEncryptedAESKey) {
        console.warn(
          `[Doctor Records API] Record ${rec._id} has no encrypted key for doctor ${doctorChain}`
        )
      }
      ...
      return {
        ...base,
        cid: cid != null ? String(cid) : base.cid,
        myEncryptedAESKey,  // ← CRITICAL: This is what doctor frontend expects
      }
    })
    ...
    console.log(`[Doctor Records API] Returning ${mapped.length} records to doctor`)
